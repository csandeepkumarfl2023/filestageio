...
app.get('/', async (req, res) => {
  ...
  const response = await todos.find({}).sort( { orderNum: 1 } ).toArray();
  res.status(200);
  res.json(response);
});
...
app.post('/', async (req, res) => {
  ...
  const todoLength = await database.client.db('todos').collection('todos').count();
  const todo = { id: generateId(), text, completed: false, orderNum: todoLength + 1 };
  await database.client.db('todos').collection('todos').insertOne(todo);
  res.status(201);
  res.json(todo);
});
app.put('/bulkUpdate', async (req, res) => {
  try {
    const { data } = req.body;
    console.log('data', data)
    console.log('typeOfdata', typeof(data))
    for (const elem of data) {
      await database.client.db('todos').collection('todos').updateOne({ id: elem.id },
        { $set: { orderNum: elem.orderNum } });
    }    
    res.status(200);
    res.end();
  } catch (error) {
